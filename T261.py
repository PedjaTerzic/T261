...
class T261(Frame):
    ...
    def initUI(self):
        ...
        rb1 = Radiobutton(frame1, text = "2*6^n-1", variable = v, value = 1,style='My.TRadiobutton')
        rb1.pack( anchor = W )
		
        rb2 = Radiobutton(frame1, text = "2*6^n+1", variable = v, value = 2,style='My.TRadiobutton')
        rb2.pack( anchor = W )
        ...
        try:
            ...
            if n<3:
                ...
                if v.get()==1:
                    ...
                    ctr=1
                    while ctr<=n-2:
                        s=(s**6-6*s**4+9*s**2-2)%N
                        ctr=ctr+1
                    if int(s)==0:
                        value="2*6^"+str(n)+"-1 is prime"
                        res.set(self.makeAsItIs(value))
                    else:
                        value="2*6^"+str(n)+"-1 is composite"
                        res.set(self.makeAsItIs(value))
					
                else:
                    N=2*6**n+1
                    s=23463956348191699342
                    ctr=1
                    while ctr<=n-2:
                        s=(s**6-6*s**4+9*s**2-2)%N
                        ctr=ctr+1
